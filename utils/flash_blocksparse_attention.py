...
from utils.flash_blocksparse_attn_interface import (
    convert_blockmask,
    flash_blocksparse_attn_func,
)

class SparsityConfig:
    def __init__(self, block_size, num_global_blocks=1, causal=False):
        self.block_size = block_size
        self.num_global_blocks = num_global_blocks
        self.causal = causal

    def make_layout(self, max_seq_length):
        """Create a sparsity layout for the attention matrix based on the configuration."""
        # Initialize a full layout (dense)
        layout = torch.ones((max_seq_length // self.block_size, max_seq_length // self.block_size))
        
        # Apply the sparsity pattern
        if self.causal:
            # For causal attention, zero out future blocks
            for i in range(layout.shape[0]):
                layout[i, i + 1:] = 0

        # Optionally, you can define global blocks that attend to all positions
